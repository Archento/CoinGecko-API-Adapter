...
export class CoinGeckoAdapter {
  ...
  get coins () {
    ...
    return {
       ...
       * @param {string} params.order - Order results by Constants.ORDER[*]
       ...
       * @param {array|string} params.price_change_percentage - Price change percentage (1h,24h,7d,14d,30d,200d,1y)
       * @param {string} params.order - Order results by Constants.ORDER[*]
       * @param {number} params.per_page - Total results per page
       * @param {number} params.page - Page through results
      ...
      markets: (params = {}) => {
        ...
        // Check the params.price_change_percentage
        // If is string, ok. If is array, convert to string
        if (Utils.isArray(params.price_change_percentage)) {
       ...
       * @param {string} params.order [default: trust_score_desc] - Order results by Constants.ORDER.TRUST_SCORE_DESC or Constants.ORDER.VOLUME_DESC
       * @returns {ReturnObject}
       */
  ...
  get statusUpdates () {
    return {
       ...
       * @param {object} params - Parameters to pass through to the request
       * @param {number} params.category - Filter results by Constants.STATUS_UPDATE_CATEGORY[*]
       * @param {number} params.project_type - Filter results by Constants.STATUS_UPDATE_PROJECT_TYPE[*] (If left empty returns both status from coins and markets)
       * @param {number} params.per_page - Total results per page
       * @param {number} params.page - Page through results
       * @returns {ReturnObject}
       */
  ...
  get events () {
    ...
    return {
       ...
       * @param {string} params.type - Type of event (eg.‘Conference’). Use events.fetchTypes() for list of types. Or use Constants.EVENT_TYPE[*]
  ...
  _buildRequestOptions (path, params) {
    ...
    if (Utils.isObject(params)) params = this._createSearchQuery(params)
   ...
   * @function _createSearchQuery
   * @param {object} params - Object representing query strings for url parameters
   * @returns {String} SearchQuery
   */
  _createSearchQuery (params) {
    let ret = ''
  ...
  _request (path, params) {
    ...
    return sendAjax(options.method, url)
  }
}
